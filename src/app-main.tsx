  ...
  const handleShare = async () => {
    ...
    const copyToClipboardFallback = (text: string) => {
      navigator.clipboard.writeText(text).then(() => {
        setToastMsg("Rute transfer disalin ke clipboard!");
        setTimeout(() => setToastMsg(null), 3000);
      }).catch(err => {
    ...
    <div id="flowchart-export-area" className="bg-theme-card border border-theme-border rounded-[24px] p-5 sm:p-6 md:p-8 lg:p-10 relative flex flex-col overflow-hidden min-h-[480px]">
      ...
      >
        ...
        <div className={`flex-1 w-full relative ${isFullscreen ? "p-4 sm:p-6 md:p-8" : ""}`}>
          ...
          {isFullscreen && (
            <div 
            className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-1 bg-theme-card/95 backdrop-blur-md border border-theme-border p-1.5 rounded-2xl z-10 select-none animate-in fade-in slide-in-from-bottom-4" 
          ...
          >
              ...
              >
                ...
                <span className="text-xs font-bold sm:inline hidden">Pusatkan</span>
              </button>
...
function AppMain() {
  ...
  }, [isDark, accentColor, badgeBgColor]);
          ...
          <main className="md:col-span-8 flex flex-col gap-6 w-full">
            ...
            {routeData.length === 0 && (
              <div className="flex-1 flex border-2 border-dashed border-theme-border rounded-[24px] flex-col items-center justify-center p-8 text-center bg-theme-card min-h-[400px]">
                <div className="w-16 h-16 bg-theme-badge border border-theme-border rounded-full flex items-center justify-center mb-5 text-theme-accent shadow-md">
